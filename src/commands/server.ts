...
export class UserCommand extends Command {
  ...
  public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
    ...
    const serverCreationDate = DateTime.fromJSDate(guild.createdAt).setZone('Europe/Tallinn').setLocale('et').toFormat('d.MMMM yyyy HH:mm');

    const timePassedSinceCreation = humanizeDuration(guild.createdTimestamp - Date.now(), {
      language: 'et',
      round: true,
      conjunction: ' ja ',
      largest: 2,
    ...
    const { data } = await axios.get(`https://discord.de/api/guilds/1065024077884047481/widget.json`);
